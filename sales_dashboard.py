...
with tabs[0]:  # 销售概览
    ...
    with col1:
        ...
        st.plotly_chart(fig_region, use_container_width=True, config=config)
    ...
    with col2:
        ...
        st.plotly_chart(fig_region_pie, use_container_width=True, config=config)
    ...
    with col1:
        ...
        st.plotly_chart(fig_packaging, use_container_width=True, config=config)
    ...
    with col2:
        ...
        st.plotly_chart(fig_price_qty, use_container_width=True, config=config)
    ...
    st.plotly_chart(fig_applicant, use_container_width=True, config=config)
...
with tabs[1]:  # 新品分析
    ...
    with col2:
        ...
        <div class="card">
            ...
            <div class="metric-value">{format_percent(new_products_percentage)}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    if not filtered_new_products_df.empty:
        ...
        st.plotly_chart(fig_product_sales, use_container_width=True, config=config)
        ...
        with col1:
            ...
            st.plotly_chart(fig_region_product, use_container_width=True, config=config)
        ...
        with col2:
            ...
            st.plotly_chart(fig_new_vs_old, use_container_width=True, config=config)
        ...
        for i in range(len(pivot_percentage.index)):
            for j in range(len(pivot_percentage.columns)):
                fig_heatmap.add_annotation(
                    x=j,
                    y=i,
                    text=f"{pivot_percentage.iloc[i, j]:.2f}%",  # 修改为保留2位小数
                    showarrow=False,
...
with tabs[2]:  # 客户细分
    ...
    if not filtered_df.empty:
        ...
        st.plotly_chart(fig_customer_types, use_container_width=True, config=config)
        ...
        for i, row in simple_segments.iterrows():
            ...
            fig.add_trace(
                go.Bar(
                    ...
                    text=[f"{format_percent(avg_new_ratio)}"],  # 使用格式化函数
                    textposition='outside',
                    textfont=dict(size=14)
                ),
                row=1, col=2
        ...
        for i, row in top_acceptance.iterrows():
            ...
            fig_top_acceptance.add_trace(go.Bar(
                ...
                text=[f"{format_percent(ratio)}"],  # 使用格式化函数
                textposition='outside',
                textfont=dict(size=14)
            ))
        ...
        st.plotly_chart(fig_top_acceptance, use_container_width=True, config=config)
...
with tabs[3]:  # 产品组合
    ...
    if not filtered_df.empty and len(filtered_df['客户简称'].unique()) > 1 and len(
        ...
        if valid_new_products:
            ...
            st.plotly_chart(fig_co_heatmap, use_container_width=True, config=config)
        else:
        ...
        with col2:
            ...
            <div class="card">
                ...
                <div class="metric-value">{format_percent(percentage_orders_with_new)}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        st.plotly_chart(fig_products_dist, use_container_width=True, config=config)
    ...
    if not filtered_df.empty:
        ...
        with col3:
            ...
            <div class="card">
                ...
                <div class="metric-value">{format_percent(penetration_rate)}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        if 'selected_regions' in locals() and selected_regions:
            ...
            for i, row in region_penetration.iterrows():
                ...
                fig_region_penetration.add_trace(go.Bar(
                    ...
                    name=region,
                    marker_color=colors[color_idx],
                    text=[f"{format_percent(penetration)}"],  # 使用格式化函数
                    textposition='outside',
                    textfont=dict(size=14)
                ))
            ...
            st.plotly_chart(fig_region_penetration, use_container_width=True, config=config)
            ...
            fig_penetration = px.bar(
                ...
                x='所属区域',
                y='渗透率',
                text=region_penetration['渗透率'].apply(lambda x: f"{format_percent(x)}"),  # 使用格式化函数
                color='所属区域',
                title='各区域新品渗透率',
            ...
            st.plotly_chart(fig_penetration, use_container_width=True, config=config)
            ...
            st.plotly_chart(fig_bubble, use_container_width=True, config=config)
        else:
        ...
        if '发运月份' in filtered_df.columns:
            ...
            try:
                ...
                fig_trend.update_traces(
                    text=[f"{format_percent(x)}" for x in monthly_penetration['渗透率']],  # 使用格式化函数
